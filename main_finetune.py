...
def main(args):
    ...
    # Init Model（仅修改这3行！补充pretrained=False，确保公平性）
    if args.model == 'SAFE':
    ...
    eff_batch_size = args.batch_size * args.update_freq * utils.get_world_size() if args.distributed else args.batch_size * args.update_freq
    ...
    if args.layer_decay < 1.0 or args.layer_decay > 1.0:
        ...
        if args.layer_decay_type == 'group': # applies for Base and Large models
            ...
            try:
                ...
                num_layers = 12  # 兼容resnet18无depths属性
            print("--------------------------------------")
            print(num_layers)
            print("--------------------------------------")
        assigner = LayerDecayValueAssigner(
            list(args.layer_decay ** (num_layers + 1 - i) for i in range(num_layers + 2)),
            depths=model_without_ddp.depths if hasattr(model_without_ddp, 'depths') else None, 
