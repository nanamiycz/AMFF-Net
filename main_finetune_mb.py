...
# ================= [Modified] Import your new model file =================
...
def get_args_parser():
    ...
    # 🔥 [New Argument] Ablation Mode (White-list Expanded!) 🔥
    parser.add_argument('--ablation_mode', default='ours', type=str,
                        choices=['baseline', 'naive', 'ours', 'll_only', 'lh_hl_only', 'all_bands'],
                        help='Ablation mode: baseline (HH only), naive (MB concat), ours (MB+Attn), ll_only, lh_hl_only, all_bands')
...
def main(args):
    ...
    # ================= [Modified] Register SAFE_MB Model =================
    elif args.model == 'SAFE_MB':
        # Pass ablation_mode to the model
        model = models.safe_mb.resnet18(num_classes=2, ablation_mode=args.ablation_mode)
        print(f">>> Model initialized: SAFE_MB (Mode: {args.ablation_mode}) with ResNet18 Backbone")
    # ===============================================================
    ...
    if args.eval:
        ...
        if args.eval_data_path and os.path.exists(args.eval_data_path):
            print(f"Evaluatin on single dataset: {args.eval_data_path}")
            pass

        if os.path.isdir(args.eval_data_path):
             subdirs = sorted([d for d in os.listdir(args.eval_data_path) if os.path.isdir(os.path.join(args.eval_data_path, d))])
             if not subdirs:
                 subdirs = [os.path.basename(args.eval_data_path)]
                 ROOT = os.path.dirname(args.eval_data_path)
             else:
                 ROOT = args.eval_data_path
             print(f"Found sub-datasets to evaluate: {subdirs}")
        else:
            print("Invalid eval path")
            return

        rows = [["{} model testing on...".format(args.resume)],
            ['testset', 'accuracy', 'avg precision']]

        for v_id, val in enumerate(subdirs):
            
            full_path = os.path.join(ROOT, val)
            if not os.path.exists(full_path): continue

            args.eval_data_path = full_path
            dataset_val = TrainDataset(is_train=False, args=args)
        ...
        csv_name = os.path.join(args.output_dir, f'eval_results_{args.ablation_mode}.csv')
